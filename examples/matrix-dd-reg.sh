...
#PBS -l nodes=2:ppn=2          # Number of nodes, use 8 processes on each
...
INPUT_BASENAME=matrix-dd-reg         # JOB NAME - USER INPUT PARAMETER
JOB_FILE=$INPUT_BASENAME.R
OUT_FILE=$INPUT_BASENAME.Rout

mpirun -np 4 Rscript $JOB_FILE > $OUT_FILE
